...
function App() {
  return (
    <div className="bg-main text-white space-y-6">
      <div className=" bg-yacht-Img bg-no-repeat bg-center ">
      ...
      <footer className="max-w-4xl mx-auto p-3 space-y-6">
        {/* links */}
        <ul className="flex flex-wrap justify-between sm:font-medium sm:text-lg">
          {links.map((link) => (
            <li key={link.link} className="m-2">
        ...
        <div className="flex flex-col sm:flex-row space-y-6 sm:space-y-0">
          <select className="bg-main text-gray-400 border-[1px] border-mainBorder px-2 h-9 focus:border-sky-500 focus:border focus:outline-none sm:font-semibold rounded mr-6">
